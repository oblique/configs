...
tags = {
    names = { "1:sh", "2:chat", "3:net", "4:code", "5:man", 6, 7, 8, 9  },
    layout = { layouts[4], layouts[1], layouts[1], layouts[2], layouts[1], layouts[1], layouts[1], layouts[1], layouts[1] }
}
...
volicon.image = image(awful.util.getdir("config") .. "/themes/daes-ob/icons/vol.png")
-- }}}
...
--            return awful.widget.tasklist.label.currenttags(c, s)
    ...
    --taskwibox[s] = awful.wibox({ position = "bottom", screen = s, height = 20 })
...
globalkeys = awful.util.table.join(
    ...
    -- modkey + shift + left, focus left window
    -- modkey + shift + right, focus right window
    -- modkey + shift + down, focus down window
    -- modkey + shift + up, focus up window
...
--        function () mypromptbox[mouse.screen]:run() end),
        function ()
            awful.util.spawn("dmenu_run -fn '-*-*-medium-r-normal-*-*-*-*-*-*-80-*-*' -i -p 'Run command:' -nb '" .. 
...
r.run("xsettingsd")
r.run("arpdetective")
r.run("wicd-client --tray")
r.run("mpd")
r.run("syndaemon -i 1.0 -k -R")
